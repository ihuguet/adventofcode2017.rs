...
fn input_to_ints(input: &str) -> Vec<usize> {
    input.split(",").map(|s| s.parse::<usize>().unwrap()).collect()
}

fn input_to_ascii(input: &str) -> Vec<usize> {
    let mut lens: Vec<usize> = input.chars().map(|c| c as usize).collect();
    lens.extend(vec![17, 31, 73, 47, 23]);
    lens
}

fn apply_dense_knot_tie_hash(mut list: Vec<u32>, lens: &[usize]) -> String {
...
fn apply_knot_tie_hash(mut list: &mut Vec<u32>, lens: &[usize], mut rounds: usize) {
    let mut pos = 0;
    let mut skip = 0;
